...
import { Activity, ChevronRight, X, Trophy, CheckCircle2, AlertCircle, Lock, LogOut, Mail, User, Key, ArrowLeft, Settings, Plus, Trash2, Calendar, RefreshCw, Shield, Volume2, VolumeX, Eye, EyeOff, Edit2, Camera, Sun, Moon, Users, Filter, Menu, Zap, HelpCircle, FileText, Upload, Download, ClipboardList, Hash, Coins, Bus, MonitorPlay, TrendingUp } from 'lucide-react';
...
const getLineColor = (index, isContrast) => {
  const colors = [
    '#10b981', // Emerald
    '#3b82f6', // Blue
    '#f59e0b', // Amber
  ...
  const contrastColors = ['#000000', '#e60000', '#0000ff', '#008000', '#800080', '#ff00ff'];
  
  const palette = isContrast ? contrastColors : colors;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-2 bg-black/90 backdrop-blur-md">
      <div className={`w-full max-w-4xl rounded-xl border shadow-2xl overflow-hidden flex flex-col max-h-[90vh] ${
        ...
        <div className={`flex-1 p-4 ${isContrast ? 'bg-white' : isDark ? 'bg-slate-900' : 'bg-white'}`}>
           {loading ? <div className="flex h-full items-center justify-center text-slate-500">Calculating stats...</div> : (
             <div className="w-full h-64 md:h-96">
               <ResponsiveContainer width="100%" height="100%">
                 <LineChart data={graphData}>
                   <CartesianGrid strokeDasharray="3 3" stroke={isContrast ? '#000' : isDark ? '#334155' : '#e2e8f0'} />
                   <XAxis 
                      dataKey="date" 
                      stroke={isContrast ? '#000' : isDark ? '#94a3b8' : '#64748b'} 
                   ...
                   <Tooltip 
                      contentStyle={{ 
                        backgroundColor: isContrast ? '#fff' : isDark ? '#1e293b' : '#fff', 
                        border: isContrast ? '2px solid #000' : '1px solid #475569',
                        borderRadius: '8px',
                        fontSize: '12px'
                      }}
                      itemStyle={{ color: isContrast ? '#000' : isDark ? '#fff' : '#000' }}
                   />
                   ...
                   {filteredUsers.slice(0, 10).map((user, index) => (
                     <Line 
                       ...
                       stroke={getLineColor(index, isContrast)} 
                       strokeWidth={isContrast ? 3 : 2}
...
const MatchPredictionsModal = ({ match, onClose, theme }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/80 backdrop-blur-sm">
      <div className={`w-full max-w-md rounded-xl border shadow-2xl overflow-hidden flex flex-col max-h-[80vh] ${isContrast ? 'bg-white border-2 border-black' : isDark ? 'bg-slate-900 border-slate-700' : 'bg-white border-gray-200'}`}>
        <div className={`p-4 flex justify-between items-center border-b ${isContrast ? 'bg-white border-black' : isDark ? 'bg-slate-800 border-slate-800' : 'bg-gray-50 border-gray-100'}`}>
           ...
           {loading ? <div className="text-center py-4 text-slate-500">Loading predictions...</div> : (
             <div className="space-y-2">
                ...
                {matchPreds.map((p, idx) => (
                  <div key={idx} className={`p-2 rounded flex justify-between items-center ${isContrast ? 'border border-black' : isDark ? 'bg-slate-800' : 'bg-gray-100'}`}>
                    <div className={`font-medium ${isContrast ? 'text-black' : isDark ? 'text-white' : 'text-gray-900'}`}>
                      {p.userName || 'Unknown'}
                    </div>
                    <div className="text-right">
                      <div className={`font-bold font-mono ${isContrast ? 'text-black' : 'text-emerald-600'}`}>
                      ...
                      {match.status === 'COMPLETED' && (
                        <div className={`text-[10px] ${isContrast ? 'text-black font-bold' : 'text-slate-500'}`}>{p.points} pts</div>
                      )}
                    </div>
                  </div>
                ))}
    ...
    <div className={`pb-24 min-h-screen ${isContrast ? 'bg-white' : isDark ? 'bg-slate-950' : 'bg-gray-100'}`}>
      ...
      <header className={`sticky top-0 z-20 backdrop-blur-md border-b px-4 py-3 flex justify-between items-center shadow-lg relative ${isContrast ? 'bg-white border-black border-b-2' : isDark ? 'bg-slate-900/80 border-slate-800' : 'bg-white/80 border-gray-200'}`}>
        <div className="flex items-center gap-2">
          ...
          <h1 className={`text-sm md:text-lg font-bold tracking-wide truncate ${isContrast ? 'text-black' : isDark ? 'text-white' : 'text-gray-900'}`}>Mesh <span className="text-emerald-500">Predictor</span></h1>
        </div>
